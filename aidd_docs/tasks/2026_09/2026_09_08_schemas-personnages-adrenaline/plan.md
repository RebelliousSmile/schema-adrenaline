...
| Chaque schéma porte un `$id` stable, posé via `.meta({ $id })` sur l'objet racine. | Sans `$id`, un schéma n'a pas d'identité propre et ne peut être référencé par un outil tiers. Vérifié sur Zod 4.3.6 : `.meta({ $id })` typecheck en strict, émet bien `$id` en draft-7, et Ajv le compile. À l'inverse `.meta({ id })` émet `id`, une clé draft-4 qu'Ajv ignore en silence. |
