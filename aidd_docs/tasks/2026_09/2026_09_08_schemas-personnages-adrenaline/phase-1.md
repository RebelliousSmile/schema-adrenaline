...
# Instruction: Référence des fiches de personnage
...
└── aidd_docs/tasks/2026_09/2026_09_08_schemas-personnages-adrenaline/
    └── reference-fiches.md   ✅ faits de structure, chacun sourcé par son fichier markdown ou sa page de PDF
```
...
```mermaid
flowchart TD
  A[Résumés markdown déjà extraits] --> C[Repérage des blocs de fiche]
  Z[PDF, seulement sur divergence] --> B[pdftotext -layout -enc UTF-8]
  B --> C
  C --> D[reference-fiches.md, chaque fait avec sa source]
...
1. Lire d'abord `_sources/regles/part_01_resume.md` (création de PJ), `part_06_resume.md` (zombiologie), `template_pnj.md` (forme de la fiche de PNJ, champ par champ) et `adrenaline-d100.md` (le socle d100 en cheatsheet).
...
### `3)` Figer la structure de la fiche de créature

> Le générateur zombie est un cas particulier ; il faut la forme, pas le bestiaire.

1. Relever les champs de la fiche : corps, état stimulé, type d'infecté, comportement, zone de détection, déplacement, SP, ND.
2. Noter quelles caractéristiques une fiche de zombie porte réellement, et lesquelles il omet.
...
4. Reprendre la forme de la fiche de PNJ de `template_pnj.md` — elle est déjà écrite champ par champ — et ne rouvrir le livret PNJ que pour vérifier ce qu'elle omet.

### `4)` Écrire la référence

> Un document que les phases suivantes lisent seul, sans revenir au PDF.

1. Écrire `reference-fiches.md` dans le dossier de la tâche.
...
| 3    | Les champs de la fiche de créature sont listés, et le document dit lesquelles des huit caractéristiques une fiche de zombie porte effectivement.  |
| 4    | `reference-fiches.md` existe, chaque affirmation porte sa source — fichier markdown ou page de PDF — et les incertitudes restantes sont nommées comme telles. |
