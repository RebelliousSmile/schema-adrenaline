...
| Construire tous les objets Zod avec `z.strictObject` et l'interdire ensuite à l'audit sous sa forme permissive `z.object`. | Les JSON Schemas générés déclarent déjà `additionalProperties: false` à chaque niveau ; le contrat exécutable doit refuser les mêmes clés au lieu de les supprimer silencieusement. |
